...
// Vector Icon Helper
#let icon(path, height: 8.5pt, baseline: 18%) = box(
  baseline: baseline,
  height: height,
)[#image("icons/" + path)]
...
#let tag(name, domain: auto) = {
  ...
  box(
    ...
    radius: 3pt,
    inset: (x: 3.5pt, y: 1.6pt),
    outset: 0pt,
    baseline: 0%,
  )[#text(size: 7.6pt, font: ("JetBrains Mono", "DejaVu Sans Mono"), weight: "medium", fill: c.text)[#name]]
}
...
#let domain-node-color(company) = {
  let c = lower(company)
  if "unowhy" in c { rgb("#1E40AF") } // Indigo
    ...
    #grid(
      ...
      ..steps.map(step => {
        ...
        box(
          ...
          #align(center + horizon)[#text(size: 6.1pt, weight: "bold", fill: c.text)[#step.label]]
        ]
      }).intersperse(text(size: 6.8pt, fill: rgb("#94A3B8"))[▶])
    )
  ]
}
...
#let cv-entry(
  ...
  grid(
    ...
    [
      #block(
        stroke: (left: if not is-last { 1.2pt + rgb("#E2E8F0") } else { 0pt }),
        inset: (left: 9pt, bottom: if not is-last { 5.5pt } else { 1pt }),
      )[
        #place(top + left, dx: -12.6pt, dy: 1.8pt)[
          #circle(radius: 3.5pt, fill: node-col, stroke: 1.5pt + rgb("#FFFFFF"))
        ]
        #text(weight: "bold", size: 9.5pt, fill: color-title)[#role]
        #text(weight: "bold", size: 9.5pt, fill: color-brand)[ — #company]
        #if location != "" [
          #text(style: "italic", size: 8.6pt, fill: color-muted)[ (#location)]
        ]
...
#let skill-gauge(name, years, max-years: 20, domain: auto) = {
  ...
  grid(
    ...
    [#text(size: 8pt, weight: "bold", fill: color-title)[#name]],
    [
      #box(width: 100%, height: 4pt, fill: rgb("#F1F5F9"), radius: 2pt)[
        #box(width: (years / max-years) * 100%, height: 4pt, fill: c.text, radius: 2pt)
      ]
    ],
    [#text(size: 7.6pt, weight: "medium", fill: color-muted)[#years ans]]
  )
}
...
#align(center)[
  ...
  #text(size: 9.0pt, fill: color-muted)[ • Backend, Architecture, SIG, 3D Temps-Réel, Python, C++]
